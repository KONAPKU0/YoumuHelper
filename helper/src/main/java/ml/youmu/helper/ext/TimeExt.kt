...
@SuppressLint("SimpleDateFormat")
fun Int.toDateStr(pattern: String = "yyyy/MM/dd HH:mm:ss"): String {
...
fun differentDays(date1: Date?, date2: Date?): Int {
    ...
    {
//        LogCat.i("判断day2(${day2}) - day1(${day1}) : " + (day2 - day1))
        day2 - day1
    }
}
...
fun Long.toDiscoveryFormat():String{
    val date = Date(this)
    val calendar = Calendar.getInstance()
    calendar.time = date
    ...
    val ago = now - time
    return if (ago <= ONE_HOUR) {
        "${ago / ONE_MINUTE}分钟前"
    } else if (ago <= ONE_DAY) {
        "${ago / ONE_HOUR}小时${ago % ONE_HOUR / ONE_MINUTE}分钟前"
    } else if (ago <= ONE_DAY * 2) {
//        "昨天" + calendar[Calendar.HOUR_OF_DAY].toString() + "点" + calendar[Calendar.MINUTE].toString() + "分"
        "昨天"
    } else if (ago <= ONE_DAY * 3) {
//        "前天" + calendar[Calendar.HOUR_OF_DAY].toString() + "点" + calendar[Calendar.MINUTE].toString() + "分"
        "前天"
    } else if (ago <= ONE_MONTH) {
        val day = ago / ONE_DAY
        if (day < 7) day.toString() + "天前" else if (day >= 7 && day < 14) "1周前" else if (day >= 14 && day < 21) "2周前" else if (day >= 21 && day < 28) "3周前" else "4周前"
    } else if (ago <= ONE_YEAR) {
        val month = ago / ONE_MONTH
        val day = ago % ONE_MONTH / ONE_DAY
//        month.toString() + "个月" + day + "天前"
        month.toString() + "个月前"
    } else {
        val year = ago / ONE_YEAR
        val month = calendar[Calendar.MONTH] + 1 // JANUARY which is 0 so month+1
//        (year.toString() + "年前" + month + "月" + calendar[Calendar.DATE] + "日")
        (year.toString() + "年前")
    }

}
fun Int.toDiscoveryFormat():String{
    val date = Date(this* 1000L)
    val calendar = Calendar.getInstance()
    calendar.time = date
